...
with tab5:
    ...
    cols[3].metric("Pintar", f"{st.session_state.pintar}")

    # 5. FUNGSI AKSI DENGAN ALASAN
    def aksi(nama, h_c, xp_c, l_c, b_c, k_c, le_c, p_c, deskripsi):
        if st.session_state.sakit and nama != "Obat":
            ...
            st.info(f"{nama}: {deskripsi}")
            st.rerun()

    # 6. TOMBOL AKSI (Gunakan Expander agar tidak panjang di HP)
    with st.expander("🎮 Pilih Aktivitas (Klik untuk Buka)", expanded=True):
        col_a, col_b = st.columns(2)
        # Menambahkan parameter deskripsi di akhir
        if col_a.button("🍼 Makan"): aksi("Makan", 5, 5, -30, 0, 0, 0, 0, "Makan agar tidak lemas.")
        if col_b.button("⚽ Main"): aksi("Main", -2, 10, 5, -40, 0, 5, 0, "Biar tidak bosan.")
        if col_a.button("📖 Belajar"): aksi("Belajar", -5, 15, 5, 5, 0, 10, 20, "Belajar buat ujian besok!")
        if col_b.button("🏃 Lari"): aksi("Lari", -8, 12, 10, -50, 10, 20, 0, "Lari agar tidak jadi remaja jompo!")
        if col_a.button("💊 Obat"): aksi("Obat", 20, -5, 0, 0, 0, 0, 0, "Obat biar cepat sembuh.")
        if col_b.button("💤 Bobo"): aksi("Bobo", 10, 2, 5, 0, 0, -50, 0, "Istirahat biar fresh.")
