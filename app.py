...
with tab5:
    ...
    def update_puyo(nama, h_c, xp_c, l_c, b_c, k_c, p_c, msg):
        ...
        # Penyakit Random (5% chance)
        if not st.session_state.sakit and random.random() < 0.05:
            st.session_state.sakit = True
            st.warning("⚠️ Puyo tiba-tiba demam! Butuh Obat!")

        # Level Up Logic
        if st.session_state.xp >= 100 * st.session_state.level:
            st.session_state.level += 1
            st.balloons()
            st.success(f"🎉 LEVEL UP! Puyo sekarang Level {st.session_state.level}")
